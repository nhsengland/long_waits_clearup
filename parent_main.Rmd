...
Data for this report was retrieved on `r paste0(day(today()),'-',month(today(),label=TRUE,abbr=TRUE),'-',year(today()))` and presents data submitted to the Waiting List Minimum Data Set (WLMDS). The data covers the period from the week ending `r paste0(day(min_date),'-',month(min_date,label=TRUE,abbr=TRUE),'-',year(min_date))` until week ending `r paste0(day(max_date),'-',month(max_date,label=TRUE,abbr=TRUE),'-',year(max_date))`.
